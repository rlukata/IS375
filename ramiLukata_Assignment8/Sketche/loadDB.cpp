...
// loadDB.cpp : Defines the class that connect ODBC to Sketche.

#include "stdafx.h"
#include "loadDB.h"
#include <sqlext.h>

BOOL CSketcheDoc::CHECK(SQLRETURN rc, System::String^ msg, bool printSucceededMsg, bool quit)
{
	if (SQL_SUCCEEDED(rc))
	{
...
void CSketcheDoc::status(SQLSMALLINT handleType, SQLHANDLE theHandle, int line)
{
	SQLCHAR sqlState[6];
...
void CSketcheDoc::loadDB(SQLVARCHAR* query) {
	...
	SQLCHAR* dsnName = (SQLCHAR*)"phonebook";  // MUST BE THE SAME
	...
	// some results.
	//SQLVARCHAR* query = (SQLVARCHAR*)"SELECT * from phonebook";
