...
// ramiLukata_Assignment5.cpp : main project file.
...
class loadDB {
	public:
		...
		// constructor
		//takes SQLVARCHAR* as a way to use scripts for queries
		loadDB(SQLVARCHAR* query) {
			...
			// use mySQLAssignment for sakila DB
			// use mysqlProject for world DB
			// use MySQLAssignment5 for phonebook
			SQLCHAR* dsnName = (SQLCHAR*)"phonebook";
			// MUST BE THE SAME
			...
			for (int i = 1; i <= numRows; i++)
			{
				...
				if (!SQL_SUCCEEDED(retCode)) {
					...
					Console::Write(L"And {0} is the LAST row.. we're not getting any more after this one\n", i);
				}
			}
...
int main() {
	SQLVARCHAR* script = (SQLVARCHAR*) "SELECT * FROM phonebook";
	loadDB::loadDB(script);
	cout << endl << "Welcome to phonebook database" << endl;
	bool quit = false;
	script = (SQLVARCHAR*) "SELECT * FROM phonebook WHERE phonenumber BETWEEN 206845480 AND 654135140";
	loadDB::loadDB(script);
	script = (SQLVARCHAR*) "SELECT * FROM phonebook WHERE firstName = 'Javier'";
	loadDB::loadDB(script);
	script = (SQLVARCHAR*) "SELECT * FROM phonebook WHERE lastName = 'Lukata'";
	loadDB::loadDB(script);
	script = (SQLVARCHAR*) "SELECT * FROM phonebook WHERE firstName BETWEEN 'Javier' AND 'Noor'";
	loadDB::loadDB(script);
	script = (SQLVARCHAR*) "SELECT * FROM phonebook WHERE lastName = 'Bond'";
	loadDB::loadDB(script);
	getchar();
}